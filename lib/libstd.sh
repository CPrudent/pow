...
pow_argv() {
    ...
    while :; do
        ...
        12)
            ...
            if ([ "$_key" = args_p ] || [[ ${#_argx_list_ref[@]} -eq 0 ]]); then
                [[ ${_debug_steps[args_p]:-1} -eq 0 ]] && {
                    declare -p _argv
                    [[ ${_debug_bps[args_p]} -eq 0 ]] && read
                }
    ...
    [[ ${_debug_steps[reset]:-1} -eq 0 ]] && {
        echo "reset=($_property)"
        [[ ${_debug_bps[reset]} -eq 0 ]] && read
    }
    [ "$_property" = yes ] && {
        _argv_ref=()
        _argc_ref=0
    ...
    for _key in ${!_args_n_kv[@]}; do
        ...
        *)
            ...
            [ -z "${_args_v_kv[$_key]}" ] || {
                ...
                [[ ${_debug_steps[check]:-1} -eq 0 ]] && {
                    declare -p _among_values _check_values
                    [[ ${_debug_bps[check]} -eq 0 ]] && read
                }
...
#                         false
    ...
    [[ ${_debug_steps[case]:-1} -eq 0 ]] && {
        echo "case=($_property)"
        [[ ${_debug_bps[case]} -eq 0 ]] && read
    }
    _argc_ref=0
